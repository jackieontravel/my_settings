...
ccsdk2_153()
{
    ...
    if [ "$NOT_IN_MOCK" -eq "1" ] ; then
        ...
        alias gomock='cd $mock'
        alias cp2mock='f=$target_top/builddir/.compile_env_sdk2_${sdk2_ver}.sh; tf=`mktemp`; echo -e "# Auto generated by 'cp2mock' command outside." > $tf ; type ccsdk2_153  | tail -n +2 >> $tf; sudo cp $tf $f && rm $tf'
...
ccsdk2_154()
{
    ...
    if [ "$NOT_IN_MOCK" -eq "1" ] ; then
		...
		alias bash2mock='sudo cp $HOME/github/my_settings/linux/.bashrc.mock $target_top/builddir/.bashrc; sudo cp $HOME/github/my_settings/linux/.bashrc_func.sh $target_top/builddir/; echo .bashrc copied '
		...
		# copy Mock related files to Mock, including Compiling environment and fs commands
        alias cp2mock='fs2mock; bash2mock; f=$target_top/builddir/.compile_env_sdk2.sh; tf=`mktemp`; echo -e "# Auto generated by 'cp2mock' command outside." > $tf ; type $fname | tail -n +2 >> $tf ; echo $fname >> $tf ; sudo cp $tf $f && rm $tf; echo $f generated.'
